#!/usr/bin/env python3
"""Готовая конфигурация запуска базовой модели CGNN на датасете Cora.

Задаёт набор гиперпараметров (подобранных для `dataset/out_cora`) и
запускает `trainers.train` отдельным процессом как модуль (`python -m
trainers.train ...`) из корня проекта -- это нужно, чтобы внутри
`trainers/train.py` корректно работали абсолютные импорты пакетов
(`trainers.trainer`, `models.gnn`, ...).

Запуск:
    python run_files/run_cgnn.py
"""
import sys
import os
import copy
